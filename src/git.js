(function () {
   ...
   * @param {Function} [then]
   */
  Git.prototype.getBranchList = function (then) {
    return this._run(['branch'], function (err, data) {
      then && then(err, !err && this._parseBranchList(data));
    });
  };
  ...
  Git.prototype._parseBranchList = function (branchList) {
    var line;
    var lines = branchList.trim().split('\n');
    ...
    for (var i = 0; i < lines.length; i++) {
      line = lines[i].trim().split(whitespace);

      if (line.length > 1) {
        current = line[1];
        branches.push(line[1])
      } else {
